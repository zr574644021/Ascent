...
public class ShopCarServlet extends HttpServlet{
    ...
    public void buy(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int id = Integer.parseInt(request.getParameter("id"));		//获取触发“购买”请求时传递的id参数，该参数存储的是商品在goodslist对象中存储的位置
        int many = Integer.parseInt(request.getParameter("many"));
        ShopCar sc = ProductManage.selectProById(id);
        sc.setMany(many);
        ...
        Shop myCar=new Shop();
        myCar.setBuylist(buylist); 						//将buylist对象赋值给Shop类实例中的属性
        myCar.addItem(sc);							//调用ShopCar类中addItem()方法实现商品添加操作

        session.setAttribute("buylist",buylist);
        response.sendRedirect("ShopServlet?pageNow=1&proName&proCategory");				//将请求重定向到ShopServlet
    }
    ...
    public void remove(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        response.sendRedirect("product/shopcar.jsp");
    }
